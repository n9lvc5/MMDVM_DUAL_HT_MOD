...
function getHeardList($logLines) {
	...
	foreach ($logLines as $logLine) {
		...
		$ber		= "";
		$rssi		= "";
		// Normalize modem debug-injected DMR lines so mode parsing at fixed
		// column 27 still sees "DMR Slot X" (without the "Debug: " prefix).
		if (preg_match('/^(.{27})Debug: (DMR Slot [12], (received RF voice header from .*|received RF end of voice transmission, .*|RF voice transmission lost, .*))$/', $logLine, $m) === 1) {
			$logLine = $m[1].$m[2];
		}
